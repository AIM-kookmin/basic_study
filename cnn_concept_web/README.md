...
1주차는 2026-09-16 흐름 개정판, 2주차는 2026-09-23 논문 중심 압축 개정판입니다. CNN 1·2주차를 각각 90분 수업으로 새로 구성했습니다.

## 바로 열기

- [웹북 열기](index.html): `index.html`을 Chrome 또는 Edge에서 엽니다. 인터넷 연결 없이 본문·폰트·도식·위젯이 동작합니다.
- [1주차 PDF · 28페이지](CNN_1주차_개념강의.pdf)
- [2주차 PDF · 22페이지](CNN_2주차_개념강의.pdf)
- [통합 PDF · 50페이지](CNN_개념강의_통합.pdf)
...
**2주차 — CNN 복습과 AlexNet 논문 분석.**

CNN 기초 복습 6페이지(2–7쪽) → 초록·서론과 문제 정의 → 구조·학습 → 원문 실험 표·그림 → Discussion → 네 가지 개인 정리 순서입니다. 복습 18분을 포함한 총 90분이며, 반복되는 준비·활동 페이지를 줄여 22페이지로 구성했습니다. [진행표](../week02_cnn_advanced/README.md)와 [논문 읽기 기록지](../week02_cnn_advanced/논문_읽기_기록지.md)를 함께 사용합니다.

인간 시각에서 얻은 영감을 설명하되 CNN이 인간의 뇌를 그대로 복제한 것처럼 설명하지 않습니다. 원 논문의 대상·조건과 수업용 비유의 한계를 구분합니다.

## 수업 운영

주차별 README에 합계 90분의 진행표가 있습니다. 코딩이 없어도 수업을 진행할 수 있습니다.

- 1주차 활동: 11–12쪽에서 작은 필터 손계산과 반응을 확인하고, 27쪽에서 실제 MNIST 오분류를 읽으며 다음 실험을 설계합니다(5분).
- 2주차: 8쪽 초록·서론 읽기, 10쪽 구조 그림 설명, 17쪽 Table 2 비교 조건 확인, 22쪽 개인 기록. 활동 안내는 리더 가이드에서 확인합니다.
...
- 2주차: 그림·표 아래 출처를 누르면 해당 원문 페이지로 이동합니다.
...
본문 하단과 원문 그림 아래에 해당 출처·페이지 링크를 제공합니다.
...
- `src/content.py`·`src/week1_mnist_content.py`·`src/week2_compact_content.py`: 50페이지 원고와 리더 해설 (`week2_alexnet_content.py`는 현재 판본 연결)
- `src/compact_components.py`·`compact.css`: 2주차 전용 레이아웃. `src/paper_components.py`의 수업용 SVG 도식을 함께 사용
...
python src/validate_alexnet.py
python src/build.py
