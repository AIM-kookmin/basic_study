...
expected = {'CNN_1주차_개념강의.pdf': len(W1), 'CNN_2주차_개념강의.pdf': len(W2), 'CNN_개념강의_통합.pdf': len(W1)+len(W2)}
...
with zipfile.ZipFile(student, 'w', zipfile.ZIP_DEFLATED) as archive:
    ...
    for path in sorted(web_files):
        if '리더가이드' not in path.name:
            archive.write(path, path.relative_to(ROOT).as_posix())
    worksheet=ROOT/'week02_cnn_advanced/논문_읽기_기록지.md'
    archive.write(worksheet,worksheet.relative_to(ROOT).as_posix())
